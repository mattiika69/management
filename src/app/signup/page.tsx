...
export default async function SignupPage({
  ...
  searchParams: Promise<{ next?: string; redirect?: string; email?: string }>;
}) {
  const { next, redirect: redirectParam, email } = await searchParams;
  ...
  return (
    <AuthPageShell>
      <SignupForm initialEmail={email} next={safeNext} />
    </AuthPageShell>
  );
}
